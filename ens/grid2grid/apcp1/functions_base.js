...
function getURL(plottype,metric,daterange,validhour,forecasthour,wavedecomp,level,region,field,frame){
	...
	var newurl = newurl.replace("TTT",field);
...
function showImage(){
	...
	//document.getElementById('loading').style.display = "none";
        var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.validhour,imageObj.forecasthour,imageObj.wavedecomp,imageObj.level,imageObj.region,imageObj.field,i);
...
function changePlotType(id){
	...
	else if(selected_plottype=="thresholdmean"){
                plottype_forecasthours = thresholdmean_forecasthours;
        }
	forecasthours = [];
        for(i=0; i<plottype_forecasthours.length; i++){
	if( (selected_plottype=="fhrmean") || (selected_plottype=="leaddate") || (selected_plottype=="fhrmean") || (selected_plottype=="perfdiag"))  {
        forecasthours.push({
...
function changeMetric(id){
	...
	//levels (products. thresholds)
	var selected_level = document.getElementById("level").value;
        var element = document.getElementById("level");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if(selected_metric=="ctc"){
             metric_levels = ctc_levels ;
        }
        else if(selected_metric=="reliability"){
	     metric_levels = relb_levels;
        }
	else if( selected_metric=="relp" || selected_metric=="rhist" ){
             metric_levels = relp_levels;
        }
	levels = [];
        for(i=0; i<metric_levels.length; i++){
        levels.push({
                displayName: metric_levels[i],
                name: metric_levels[i],
                })
        }
        for(i=0; i<levels.length; i++){
        var option = document.createElement("option");
...
function changeField(id){
	...
	var selected_field = document.getElementById("field").value;

        // Validation Hour
        var selected_validhour = document.getElementById("validhour").value;
        var element = document.getElementById("validhour");
        for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
        if(selected_field=="apcp_01h"){
                field_validhours = apcp_01h_validhours;
		//window.alert(field_validhours);
        }
        else if(selected_field=="apcp_03h"){
                field_validhours = apcp_03h_validhours;
		//window.alert(field_validhours);
        }
        else{
                field_validhours = apcp_24h_validhours;
		//window.alert(field_validhours);
        }
        validhours = [];
        for(i=0; i<field_validhours.length; i++){
        validhours.push({
                displayName:  field_validhours[i].toUpperCase(),
                name: field_validhours[i],
                })
        }
        for(i=0; i<validhours.length; i++){
        var option = document.createElement("option");
