		...
		for(i=0; i<plottypes.length; i++){
			...
			element.add(option);
                ...
                for(i=0; i<widths.length; i++){
                        var option = document.createElement("option");
                        option.text = widths[i].displayName;
                        option.value = widths[i].name;
                        element.add(option);
                }
        }
}
...
function getURL(plottype,metric,daterange,inithour,forecasthour,region,threshold,level,width,frame){
	...
	var newurl = newurl.replace("WWW",width);
	for(var i=0; i<5; i++){
...
function showImage(){
	...
	var url = getURL(imageObj.plottype,imageObj.metric,imageObj.daterange,imageObj.inithour,imageObj.forecasthour,imageObj.region,imageObj.threshold,imageObj.level,imageObj.width,i);
	...
	document.getElementById("width").selectedIndex = searchByName(imageObj.width,widths);
...
function changePlotType(id){
    ...
    if(selected_plottype=="fhrmean"){
            plottype_thresholds = fhrmean_thresholds;
            plottype_thresholds_name = fhrmean_thresholds_name;
    }
    else{
            plottype_thresholds = non_fhrmean_thresholds;
            plottype_thresholds_name = non_fhrmean_thresholds_name;
    }
    thresholds = [];
    ...
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_plottype=="fhrmean"){
            plottype_inithours = fhrmean_inithours;
            plottype_inithours_name = fhrmean_inithours_name;
    }
    else{
            plottype_inithours = non_fhrmean_inithours;
            plottype_inithours_name = non_fhrmean_inithours_name;
    }
    inithours = [];
...
function changeInitHour(id){
    ...
    var element = document.getElementById("forecasthour");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_inithour=="00"){
            inithour_forecasthours = z0_forecasthours;
            inithour_forecasthours_name = z0_forecasthours_name;
    }
    else if(selected_inithour=="06"){
            inithour_forecasthours = z6_forecasthours;
            inithour_forecasthours_name = z6_forecasthours_name;
    }
    else if(selected_inithour=="12"){
            inithour_forecasthours = z12_forecasthours;
            inithour_forecasthours_name = z12_forecasthours_name;
    }
    else if(selected_inithour=="12"){
            inithour_forecasthours = z18_forecasthours;
            inithour_forecasthours_name = z18_forecasthours_name;
    }
    else{
            inithour_forecasthours = na_forecasthours;
            inithour_forecasthours_name = na_forecasthours_name;
    }
    forecasthours = [];
    for(i=0; i<inithour_forecasthours.length; i++){
        forecasthours.push({
            displayName:  inithour_forecasthours_name[i],
            name: inithour_forecasthours[i],
            })
    }
    for(i=0; i<forecasthours.length; i++){
        var option = document.createElement("option");
...
//Change the width from dropdown menu
