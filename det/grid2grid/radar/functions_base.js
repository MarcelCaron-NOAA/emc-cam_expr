...
function changePlotType(id){
    ...
    if(selected_plottype=="fhrmean"){
            ...
            plottype_forecasthours_name = fhrmean_forecasthours_name;
    }
    else{
            plottype_forecasthours = non_fhrmean_forecasthours;
            plottype_forecasthours_name = non_fhrmean_forecasthours_name;
    }
    forecasthours = [];
...
function changeRegion(id){
    ...
    //Width
    var selected_width = document.getElementById("width").value;
    var element = document.getElementById("width");
    for(i = element.options.length - 1 ; i >= 0 ; i--){element.remove(i);}
    if(selected_region=="alaska"){
            region_widths = alaska_widths;
            region_widths_name = alaska_widths_name;
    }
    else{
            region_widths = non_alaska_widths;
            region_widths_name = non_alaska_widths_name;
    }
    widths = [];
    for(i=0; i<region_widths.length; i++){
        widths.push({
            displayName:  region_widths_name[i],
            name: region_widths[i],
            })
    }
    for(i=0; i<widths.length; i++){
        var option = document.createElement("option");
    ...
    var widths_values= [];
    for(i=0; i<element.options.length; i++){
        widths_values.push(element.options[i].value);
    }
    if(widths_values.indexOf(selected_width) != -1){
        var idx = widths_values.indexOf(selected_width);
        element.options[idx].selected = true;
        element.onchange();
    }
    else{
